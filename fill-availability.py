...
for event in get_appointments_in_range(0, 7 * weeks):
    # print(event.Subject, event.IsRecurring, event.GetRecurrencePattern().RecurrenceType)
    ...
    if fixed:
        print(event.Subject, event.Start.strftime(d_mon+' %H:%M'), 'busy' if busy else '',
              'oof' if oof else '', 'weekly' if weekly else '', f"{people=} {hours=:.1f}")
...
old_end_date = mod_time + pandas.to_timedelta(7 * weeks, 'day')
...
today = pandas.to_datetime('today').round('d')
...
toast = ''
if changed_to_yes:
    toast += 'Changed to Yes: ' + ', '.join(date.strftime(d_mon) for date in changed_to_yes) + '\n'
if changed_to_no:
    toast += 'Changed to No: ' + ', '.join(date.strftime(d_mon) for date in changed_to_no)
