...
async def archive_data() -> bool | None:
    ...
    workbook = read_excel(excel_filename, header=[0, 1, 2],
                          sheet_name=None,  # return a dict of all sheets
                          index_col=0)
    ...
    new_intensity_data = get_co2_data(first_new_int_time,
                                      'WA4',  # local to Daresbury Laboratory
                                      do_pivot=False,  # one timestamp per line, not one date per line
                                      end=last_new_time)
    ...
    with pandas.ExcelWriter(excel_filename, mode='a',  # append
    ...
    for sheet_name in (meters_sheet, intensity_sheet):
        ...
        for col in range(2, worksheet.max_column + 1):
            ...
            worksheet.column_dimensions[column_name].width = 18  # a bit wider than standard to ease reading titles
        for row in worksheet.iter_rows(1, 3):
            for cell in row:
                cell.font = bold_format
                if cell.row == 3:  # meter titles are in row 3
...
async def test_fetch():
    start = pandas.to_datetime('2026-03-12').to_pydatetime()
    print(start)
